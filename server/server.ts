...
function bufferToFile(buffer: Buffer, filename: string, type: string) {
...
function calculateConfidence(text: string): number {
  ...
  if (length < 5) confidence -= 0.3; // Very short text
  else if (length < 10) confidence -= 0.2; // Short text
  else if (length > 50) confidence += 0.1; // Longer text

  // Word count factor
  const words = text.split(/\s+/).filter((word) => word.length > 0);
  if (words.length < 2) confidence -= 0.2; // Single word
  else if (words.length > 10) confidence += 0.1; // Many words
  ...
  if (repetitionRatio < 0.5) confidence -= 0.2; // High repetition
  ...
  const commonWordCount = wordsArray.filter((word) =>
    commonWords.includes(word)
  ).length;
  const commonWordRatio = commonWordCount / wordsArray.length;
  if (commonWordRatio > 0.3) confidence += 0.1; // Many common words
...
function detectWrongLanguageByPatterns(
  text: string,
  ...
  if (expectedLanguage === 'en') {
    ...
    const hasKoreanRomanization = koreanRomanizationPatterns.some((pattern) =>
...
function detectWrongLanguageWords(
  text: string,
  expectedLanguage: string
): { detected: boolean; message: string } {
  ...
  for (const [lang, words] of Object.entries(languageWords)) {
    if (lang !== expectedLanguage) {
      const hasWrongLanguageWords = words.some((word) =>
        trimmedText.includes(word.toLowerCase())
      );
      ...
      if (hasWrongLanguageWords) {
        ...
        return {
          detected: true,
          message:
            messages[expectedLanguage as keyof typeof messages] ||
...
app.post('/api/prime-language', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Language context set for ${
        language === 'ky'
          ? 'Kyrgyz'
          : language === 'en'
          ? 'English'
...
app.post('/api/transcribe', upload.single('audio'), async (req, res) => {
  try {
    ...
    else if (req.file.mimetype.includes('flac')) extension = 'flac';
    else if (req.file.mimetype.includes('mpeg')) extension = 'mpeg';
    else if (req.file.mimetype.includes('mpga')) extension = 'mpga';
    else if (req.file.mimetype.includes('oga')) extension = 'oga';

    console.log(
      'Received file with mimetype:',
      req.file.mimetype,
      'using extension:',
      extension
    );
    ...
    if (text.includes('WRONG_LANGUAGE_DETECTED:')) {
      console.log(
        '⚠️ Legacy WRONG_LANGUAGE_DETECTED found in text, ignoring and continuing...'
      );
...
"OK" or "DETECTED: [language]"`;
      ...
      const detectionResult =
        detectionResponse.choices[0]?.message?.content?.trim() || 'OK';
      ...
      if (hasHangul) detectedLanguage = 'Korean';
      else if (hasChinese) detectedLanguage = 'Chinese';
      else if (hasCyrillic) detectedLanguage = 'Cyrillic script';
      else if (hasLatin) detectedLanguage = 'English/Latin script';
...
app.post('/api/tts', async (req, res) => {
  try {
    const {
      ...
      voice = 'alloy',
      format = 'mp3',
    } = req.body as { text: string; voice?: string; format?: string };
